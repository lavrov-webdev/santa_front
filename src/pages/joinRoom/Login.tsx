...
const Login: FC = () => {
  ...
  const [{data: roomInfoRes, loading}, refetch] = useGetRoomInfo(() => roomId ?? "")
  const [{loading: loginLoading}, login] = useLogin()

  const usersToSelect = useMemo(() => {
    return roomInfoRes?.data.users_to_login?.map(u => ({label: u.name, value: u.id}))
  }, [roomInfoRes])
  ...
  return (
    <div className={styles.loginWrapper}>
      {
        (loading || !usersToSelect || loginLoading) ? <Loader/> :
          roomInfoRes?.error ?
